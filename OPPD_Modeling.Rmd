...
```{r}
# Base model attempt
plants_input <- layer_input(shape = list(150,150,3), name = "plants")
...
species_prediction <- base_model %>%
  layer_dense(units = 1, activation = "softmax", name = "eppo")
condition_prediction <- base_model %>%
  layer_dense(units = 1, activation = "softmax", name = "growth_condition")

model <- keras_model(
  plants_input,
  list(species_prediction, condition_prediction)
)
