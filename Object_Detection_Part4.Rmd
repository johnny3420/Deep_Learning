...
model %>% fit(
  ...
  callbacks = callback_model_checkpoint(
    filepath = "New_SSD_Model_Weights.hdf5",
    monitor = "loss",
    save_best_only = TRUE,
    mode = "min", 
...
# loss: 5.0828 - class_output_loss: 3.3237 - bbox_output_loss: 1.7591 - class_output_class_loss: 3.3237 - bbox_output_bbox_loss: 1.7591

```{r}
test_image <- imageinfo4ssd[1, ]
tname <- (test_image$name %>% str_split(pattern = ", "))[[1]]
tx_left <- (test_image$xl %>% str_split(pattern = ", "))[[1]]
tx_right <- (test_image$xr %>% str_split(pattern = ", "))[[1]]
ty_top <- (test_image$yt %>% str_split(pattern = ", "))[[1]]
ty_bottom <- (test_image$yb %>% str_split(pattern = ", "))[[1]]
img <- image_read(file.path(img_dir, test_image$file_name))
img <- image_resize(img, geometry = "224x224!")
img <- image_draw(img)
for (i in 1:test_image$cnt) {
  rect(tx_left[i],
       ty_bottom[i],
       tx_right[i],
       ty_top[i],
       ...
       lwd = 2)
  text(
    x = as.integer(tx_right[i]),
    y = as.integer(ty_top[i]),
    labels = tname[i],
    offset = 0.25,
    pos = 3,
    cex = 1,
...
## plot prediction boxes and categories
pred <-
  model %>% predict(load_and_preprocess_image(test_image$file_name,
                                              target_height,
                                              target_width))
...
for (i in 1:16) {
  # there are 16 boxes
  pred_box <- pred[[2]][1, i, ]
  pred_cat_probs <- boot::inv.logit(pred[[1]][1, i, ])
  pred_cat <- which.max(pred_cat_probs)
  
  ### do we want to plot anything?
  if (pred_cat == 21)
    next()
  # predicted background
  
  ### get class label
  pred_name <- classes[pred_cat]
  pred_name_prob <- round(pred_cat_probs[pred_cat], 2)
  
  ### convert the box
  #### predictions are offsets from anchor centers
  #(xl, yt, xr, yb)
  
  px_left <- ((pred_box[1] + anchors[i, 1])* target_width)
  py_top <- ((pred_box[2] + anchors[i, 2])* target_height)
  px_right <- ((pred_box[3] + anchors[i, 1]) * target_width )
  py_bottom <- ((pred_box[4] + anchors[i, 2]) * target_height)
  ...
  text(
    x = as.integer(px_left),
    y = as.integer(py_top),
    labels = paste(pred_name, pred_name_prob),
    offset = 0.25,
    pos = 3,
    cex = 1,
    col = "yellow"
...
print(img)
for (i in 1:16) {
  cat(which.max(boot::inv.logit(pred[[1]][1, i, ])), "")
}
```
...
model %>% fit(
  ...
  epochs = 15,
  callbacks = callback_model_checkpoint(
    filepath = "Focal_SSD_Model_Weights.hdf5",
    monitor = "loss",
    save_best_only = TRUE,
    mode = "min", 
    save_weights_only = TRUE,
...
#loss: 1.6388 - class_output_loss: 0.1851 - bbox_output_loss: 1.4537 - class_output_class_loss_focal: 0.1851 - bbox_output_bbox_loss: 1.4537
