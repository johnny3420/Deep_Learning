...
Best_SSD_Model_Weights_2.hdf5

# Load Weights
```{r}
load_model_weights_hdf5(model, "Best_SSD_Model_Weights_2.hdf5")
```
...
for (i in 1:16) {
  ...
  px_left <- (pred_box[1] * target_width + anchors[i, 1])
  py_top <- (pred_box[2] * target_height + anchors[i, 2])
  px_right <- (pred_box[3] * target_width + anchors[i, 1])
  py_bottom <- (pred_box[4] * target_height  + anchors[i, 2])
